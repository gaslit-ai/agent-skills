...
compatibility: Requires the codex CLI (authenticated via codex login) and Python 3.10+
...
```bash
python3 scripts/codex_duo.py "<concept / idea to stress-test>"
# uv run scripts/codex_duo.py "<concept>"   # equivalent (deps are stdlib only)
```
...
Run `python3 scripts/codex_duo.py --help` for everything.
...
`assets/prompts/<name>.md` and reference them by bare name via `--system-a/-b`.